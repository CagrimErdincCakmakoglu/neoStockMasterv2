...
{
    public class HALKBANKforex
    {
        private readonly HttpClient _httpClient;

        public HALKBANKforex()
        {
        ...
        public async Task<(decimal usdBuy, decimal usdSell, decimal euroBuy, decimal euroSell)> GetExchangeRatesAsync()
        {
            try
            {
                // Halkbank döviz sayfasını getir
                var response = await _httpClient.GetAsync("https://canlidoviz.com/doviz-kurlari/halkbank");
                response.EnsureSuccessStatusCode();
                var htmlContent = await response.Content.ReadAsStringAsync();

                // USD alış değerini çek (cid="1017")
                var usdBuyStr = ExtractValue(htmlContent, "<span cid=\"1017\" dt=\"bA\"", ">", "</span>");
                var usdBuy = ParseDecimal(usdBuyStr);

                // USD satış değerini çek (cid="1017")
                var usdSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"1017\" dt=\"amount\"", ">", "</span>");
                var usdSell = ParseDecimal(usdSellStr);

                // Euro alış değerini çek (cid="1027")
                var euroBuyStr = ExtractValue(htmlContent, "<span cid=\"1027\" dt=\"bA\"", ">", "</span>");
                var euroBuy = ParseDecimal(euroBuyStr);

                // Euro satış değerini çek (cid="1027")
                var euroSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"1027\" dt=\"amount\"", ">", "</span>");
            ...
            {
                throw new Exception("Halkbank döviz kurları alınırken hata: " + ex.Message);
            }
        }
