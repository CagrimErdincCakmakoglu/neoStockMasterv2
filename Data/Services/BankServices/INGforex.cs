...
namespace neoStockMasterv2.Data.Services.BankServices
{
    public class INGforex
    {
        ...
        public async Task<(decimal usdBuy, decimal usdSell, decimal euroBuy, decimal euroSell)> GetExchangeRatesAsync()
        {
            try
            {
                // ING Bank döviz sayfasını getir
                var response = await _httpClient.GetAsync("https://canlidoviz.com/doviz-kurlari/ing-bank");
                response.EnsureSuccessStatusCode();
                var htmlContent = await response.Content.ReadAsStringAsync();

                // USD alış değerini çek (cid="1023")
                var usdBuyStr = ExtractValue(htmlContent, "<span cid=\"1023\" dt=\"bA\"", ">", "</span>");
                var usdBuy = ParseDecimal(usdBuyStr);

                // USD satış değerini çek (cid="1023")
                var usdSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"1023\" dt=\"amount\"", ">", "</span>");
                var usdSell = ParseDecimal(usdSellStr);

                // Euro alış değerini çek (cid="1033")
                var euroBuyStr = ExtractValue(htmlContent, "<span cid=\"1033\" dt=\"bA\"", ">", "</span>");
                var euroBuy = ParseDecimal(euroBuyStr);

                // Euro satış değerini çek (cid="1033")
                var euroSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"1033\" dt=\"amount\"", ">", "</span>");
            ...
            {
                throw new Exception("ING Bank döviz kurları alınırken hata: " + ex.Message);
            }
        }
