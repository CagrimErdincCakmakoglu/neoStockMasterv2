...
namespace neoStockMasterv2.Data.Services.BankServices
{
    public class MERKEZforex
    {
        ...
        public async Task<(decimal usdBuy, decimal usdSell, decimal euroBuy, decimal euroSell)> GetExchangeRatesAsync()
        {
            try
            {
                // Merkez Bankası döviz sayfasını getir
                var response = await _httpClient.GetAsync("https://canlidoviz.com/doviz-kurlari/merkez-bankasi");
                response.EnsureSuccessStatusCode();
                var htmlContent = await response.Content.ReadAsStringAsync();

                // USD alış değerini çek
                var usdBuyStr = ExtractValue(htmlContent, "<span cid=\"1016\" dt=\"bA\"", ">", "</span>");
                var usdBuy = ParseDecimal(usdBuyStr);

                // USD satış değerini çek
                var usdSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"1016\" dt=\"amount\"", ">", "</span>");
                var usdSell = ParseDecimal(usdSellStr);

                // Euro alış değerini çek
                var euroBuyStr = ExtractValue(htmlContent, "<span cid=\"1026\" dt=\"bA\"", ">", "</span>");
                var euroBuy = ParseDecimal(euroBuyStr);

                // Euro satış değerini çek
                var euroSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"1026\" dt=\"amount\"", ">", "</span>");
            ...
            {
                throw new Exception("Merkez Bankası döviz kurları alınırken hata: " + ex.Message);
            }
        }
