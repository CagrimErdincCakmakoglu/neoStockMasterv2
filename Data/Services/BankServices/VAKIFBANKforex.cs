...
{
    public class VAKIFBANKforex
    {
        private readonly HttpClient _httpClient;

        public VAKIFBANKforex()
        {
        ...
        public async Task<(decimal usdBuy, decimal usdSell, decimal euroBuy, decimal euroSell)> GetExchangeRatesAsync()
        {
            try
            {
                // VakıfBank döviz sayfasını getir
                var response = await _httpClient.GetAsync("https://canlidoviz.com/doviz-kurlari/vakifbank");
                response.EnsureSuccessStatusCode();
                var htmlContent = await response.Content.ReadAsStringAsync();

                // USD alış değerini çek (cid="1018")
                var usdBuyStr = ExtractValue(htmlContent, "<span cid=\"1018\" dt=\"bA\"", ">", "</span>");
                var usdBuy = ParseDecimal(usdBuyStr);

                // USD satış değerini çek (cid="1018")
                var usdSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"1018\" dt=\"amount\"", ">", "</span>");
                var usdSell = ParseDecimal(usdSellStr);

                // Euro alış değerini çek (cid="1028")
                var euroBuyStr = ExtractValue(htmlContent, "<span cid=\"1028\" dt=\"bA\"", ">", "</span>");
                var euroBuy = ParseDecimal(euroBuyStr);

                // Euro satış değerini çek (cid="1028")
                var euroSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"1028\" dt=\"amount\"", ">", "</span>");
            ...
            {
                throw new Exception("VakıfBank döviz kurları alınırken hata: " + ex.Message);
            }
        }
