...
{
    public class YAPIKREDIforex
    {
        private readonly HttpClient _httpClient;

        public YAPIKREDIforex()
        {
        ...
        public async Task<(decimal usdBuy, decimal usdSell, decimal euroBuy, decimal euroSell)> GetExchangeRatesAsync()
        {
            try
            {
                // Yapı Kredi döviz sayfasını getir
                var response = await _httpClient.GetAsync("https://canlidoviz.com/doviz-kurlari/yapi-kredi");
                response.EnsureSuccessStatusCode();
                var htmlContent = await response.Content.ReadAsStringAsync();

                // USD alış değerini çek (cid="819")
                var usdBuyStr = ExtractValue(htmlContent, "<span cid=\"819\" dt=\"bA\"", ">", "</span>");
                var usdBuy = ParseDecimal(usdBuyStr);

                // USD satış değerini çek (cid="819")
                var usdSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"819\" dt=\"amount\"", ">", "</span>");
                var usdSell = ParseDecimal(usdSellStr);

                // Euro alış değerini çek (cid="820")
                var euroBuyStr = ExtractValue(htmlContent, "<span cid=\"820\" dt=\"bA\"", ">", "</span>");
                var euroBuy = ParseDecimal(euroBuyStr);

                // Euro satış değerini çek (cid="820")
                var euroSellStr = ExtractValue(htmlContent, "<span itemprop=\"price\" cid=\"820\" dt=\"amount\"", ">", "</span>");
            ...
            {
                throw new Exception("Yapı Kredi döviz kurları alınırken hata: " + ex.Message);
            }
        }
